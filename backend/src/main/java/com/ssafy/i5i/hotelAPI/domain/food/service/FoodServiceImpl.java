...
public class FoodServiceImpl implements FoodService{
	...
	public List<FoodResponseDto> getFoodFromLngLat(AttractionCoordiRequestDto requestDto) {
		...
		List<FoodResponseDto>tempAroundFoodList = foodRepository.getAroundFoodList(maxY, maxX, minY, minX).orElseThrow(() -> new CommonException(ExceptionType.NULL_POINT_EXCEPTION));
		List<FoodResponseDto>resultAroundFoodList = new ArrayList<>();

		//정확한 거리 측정
		for(FoodResponseDto aroundFood : tempAroundFoodList) {
			double distance = getDistance(nowLatitude, nowLongitude, aroundFood.getFoodLatitude(), aroundFood.getFoodLongitude());
			if(distance < requestDto.getDistance()) {
				resultAroundFoodList.add(aroundFood);
			}
		}
		return resultAroundFoodList;
	}
